...
for repo in $ALL_CACHES; do
	...
	echo "❌ Removing dead cache repository: cache/$repo"
...
# Keeps images used in the last 7 days (168h)
docker system prune -af --filter "until=168h"

# 6. Clean up the isolated DinD environments
echo "------------------------------------------------"
echo "Running Docker System Prune inside all DinD containers..."

# Find all running containers using the docker:dind image
DIND_CONTAINERS=$(docker ps -q --filter "ancestor=docker:dind")

if [ -z "$DIND_CONTAINERS" ]; then
	echo "No DinD containers currently running."
else
	for container in $DIND_CONTAINERS; do
		echo "Pruning inside DinD container ID: $container"
		docker exec "$container" docker system prune -af --filter "until=168h"
	done
fi
